...
**Language:** Norwegian (comments, variable names). UI strings are internationalised — see `translations.js` below.
...
- **Codelist:** Alternative grouping of dimension values. Two types:
  - `vs_` (valueset/filter) — subset of original dimension codes; `valueCodes` in the POST body contains original codes
  - `agg_` (aggregation) — defines new aggregate codes (e.g. `"005"` = "0–4 år") that map to multiple original codes via `valueMap`; `valueCodes` must contain the **aggregate codes**, not the expanded originals — the API resolves them using the `codelist` field
...
`AppConfig` contains all tunables: API URL, cache TTLs, cell limits (defaults overridden at startup by `/config` endpoint), UI debounce timings, export defaults, SSB update schedule. Also holds instance-specific branding:

- **`source`** — Data source attribution (name, URL, license). Rendered in the footer.
- **`app`** — App name and tagline (tagline can be a `{ nb, en }` object).
- **`languages`** — Array of `{ code, apiLang, label }`. `code` is the UI language key; `apiLang` is sent as `lang=` to the API. First entry is the default.

### Internationalisation (`translations.js`)

All UI strings go through `t(key)` / `tpl(key, ...args)` — never hardcode Norwegian text in view files. `setLanguage(code)` switches language and persists to `localStorage`. `getCurrentApiLang()` returns the correct `lang=` value for the active language. Instance-specific strings (app name, source name) live in `AppConfig`, not in the translation dictionary.

## CSS

Plain CSS split across three files. No preprocessor.

- **`css/main.css`** — Global layout, typography, CSS variables (`--color-*`, `--spacing-*`), header, footer, buttons, forms.
- **`css/table.css`** — Data table display: container, controls bar, cell formatting, status symbols.
- **`css/menu-navigator.css`** — Front page, search, topic browser, navigation elements.
